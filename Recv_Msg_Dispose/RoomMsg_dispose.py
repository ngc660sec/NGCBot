...
class RoomMsg_disposes:
    ...
    def Administrator_Function(self, ws):
        ...
        if '\u2005' in self.msgJson['content'] and \
            ...
            self.pool.submit(self.Tf.add_admin, ws, self.at_wxid, self.at_nickname, self.roomid, self.room_name, self.senderid, self.nickname)
        # 删除管理员
        elif '\u2005' in self.msgJson['content'] and \
                self.judge_keyword(keyword=self.keyword.replace('@', '').replace(self.at_nickname, ''),
                                   custom_keyword=self.del_admin_words):
            self.pool.submit(self.Tf.del_admin, ws, self.at_wxid, self.at_nickname, self.roomid, self.senderid, self.nickname)
    ...
    def Integral_Function(self, ws):
        ...
        # 积分加减操作
        self.pool.submit(self.Tf.judge_operation, ws, self.keyword, self.senderid, self.at_wxid, self.at_nickname, self.roomid, self.nickname)
